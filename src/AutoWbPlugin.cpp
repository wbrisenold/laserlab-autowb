...
AutoWbPlugin::AutoWbPlugin(OfxImageEffectHandle h)
    : OFX::ImageEffect(h), m_NeedAnalyze(false) {
...
// One-shot analysis on THIS frame only.
    if (m_NeedAnalyze) {
        ...
        if (src) {
            ...
            if (W > 0 && H > 0) {
                LaserLabParams sp = scanParams(a.time);
                if (a.isEnabledMetalRender && a.pMetalCmdQ) {
                    // GPU scan: measure skin on the current frame.
                    std::vector<float> stats((size_t)kGridW * kGridH * 4);
                    ...
                    if (n >= 32 && sw > 1e-6f) {
                        float temp = 0, tint = 0;
                        laserlabcore::solveSkinTempTint((float)(sL/sw), (float)(sa/sw), (float)(sb/sw), temp, tint);
                        setSolvedWb(temp, tint, n);
                    } else {
                        reportNoSkin();
                    }
                } else {
                    // CPU fallback analyzer in the core.
